 ...
 * Markdor
 *
 * Markdor's primary goal is to be easy to adapt. It aims to be compliant with
 * John's Gruber's [Markdown Syntax page][1],
 * but compatibility with other markdown implementations' edges cases will be
 * sacrificed where it conflict with simplicity or extensibility
 *
 * If you have struggled to customize an existing library to meet your needs, markdor might be the help.
 *
 * Many of the regexes and original login has been adapted from
 * the wonderful [marked.js](https://github.com/chjj/marked)
 */

import type * as React from "react";
import { Capture, MatchFunction, State } from "./types";
...
type ReactOutput = Output<ReactElement>;
...
type ReactOutputRule = {
  ...
  // we don't use ? because this makes it be expilicitly defined as either
...
type ArrayRule = {
  // @ts-expect-error Property 'react' of type 'ArrayNodeOutput<React.ReactElement<any>> | undefined' is not assignable to 'string' index type 'ArrayNodeOutput<any>'
  readonly react?: ArrayNodeOutput<ReactElement>;
  // @ts-expect-error Property 'html' of type 'ArrayNodeOutput<string> | undefined' is not assignable to 'string' index type 'ArrayNodeOutput<any>'
  readonly html?: ArrayNodeOutput<string>;
  readonly [key: string]: ArrayNodeOutput<any>;
};

type ParserRules = {
  // @ts-expect-error Property 'Array' of type 'ArrayRule | undefined' is not assignable to 'string' index type 'ParserRule'
...
type ReactRules = {
  // @ts-expect-error Property 'Array' of type '{ readonly react: ArrayNodeOutput<React.ReactElement<any>>; } | undefined' is not assignable to 'string' index type 'ParserRule & HtmlOutputRule'
  readonly Array?: {
    readonly react: ArrayNodeOutput<ReactElement>;
  };
...
type HtmlRules = {
  // @ts-expect-error Property 'Array' of type '{ readonly html: ArrayNodeOutput<string>; } | undefined' is not assignable to 'string' index type 'ParserRule & HtmlOutputRule'
  readonly Array?: {
    readonly html: ArrayNodeOutput<string>;
  };
...
// reuse defaultRules built-in. So we make stronger guarantees when
// we can:
...
/**
 * Turn various whitespace into easy-to-process white space
 */
 ...
 *
 * @param {Markdor.ParserRules} rules
 *      an object containing
 *      rule type -> {match, order, parse} objects
 *      (lower order is higher precedence)
 * @param {Markdor.OptionalState}[defaultState]
 *
 * @returns {Markdor.Parser}
 *      The resulting parse function, with the following parameters:
 *      @source: the input source string to be parsed
 *      @state: an optional object to be threaded through parse
 *              calls. Allows clients to add stateful operations to
 *              parsing, such is keeping track of how many levels deep
 *              some nesting is.
 */
...
// Creates a match function for inline scoped element from a regex
var inlineRegex = function (regex: RegExp): MatchFunction {
  var match = function (
    ...
    // @ts-expect-error roperty 'regex' does not exist on type '(source: string, state: State, prevCapture: string) => Capture | null | undefined'
    match.regex = regex;
  };
...
var anyScopeRegex = function (regex: RegExp): MatchFunction {
  var match: MatchFunction = function (source, _state) {
    return regex.exec(source);
  };
 ...
 * @param {{ [attr: string]: Markdor.Attr }} [attributes] - Optional extra attributes of tag as an object of key-value pairs
...
var SANITIZE_TEXT_CODES: { [key: string]: string } = {
  "<": "&lt;",
  ">": "&gt;",
  "&": "&amp;",
  '"': "&quot;",
...
var TABLES = (function () {
  ...
  /**
   * @param {string} source
   * @param {Markdor.Parser} parse
   * @param {Markdor.State} state
   * @param {boolean} trimEndSeparators
   * @returns {Markdor.ASTNode[][]}
   */
  ...
  /**
   * @param {boolean} trimEndSeparators
   * @returns {Markdor.SingleNodeParseFunction}
   */
...
var outputFor = function <Rule>(
  ...
  var latestState: State;
  var arrayRule: any = rules.Array || defaultRules.Array;
...
var defaultReactOutput: ReactOutput = outputFor(defaultRules as any, "react");
var defaultHtmlOutput: HtmlOutput = outputFor(defaultRules as any, "html");
